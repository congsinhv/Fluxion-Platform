"""Auth helpers: AppSync identity -> User row; DPC api_key bearer -> Device row."""
...
def get_user_from_identity(db: Database, identity: dict | None) -> dict | None:
    """Resolve the admin user row by Cognito subject (read-only)."""
    if not identity:
        return None
    sub = identity.get("sub") or identity.get("username")
    if not sub:
        return None
    return db.get_user_by_cognito_sub(sub)
